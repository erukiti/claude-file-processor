...
describe('file utils', () => {
  describe('readFileContent', () => {
    ...
    it('should throw FileOperationError when read fails', async () => {
      ...
      (readFile as unknown as ReturnType<typeof vi.fn>).mockRejectedValue(mockError);

      await expect(readFileContent('test.txt')).rejects.toThrow('File operation failed at test.txt: Read error');
    });
  });
  ...
  describe('writeFileContent', () => {
    it('should write file content correctly', async () => {
      ...
      expect(writeFile).toHaveBeenCalledWith('test.txt', 'test content', 'utf-8');
    });

    it('should throw FileOperationError when write fails', async () => {
      const mockError = new Error('Write error');
      (mkdir as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
      (writeFile as unknown as ReturnType<typeof vi.fn>).mockRejectedValue(mockError);

      await expect(writeFileContent('test.txt', 'test content')).rejects.toThrow('File operation failed at test.txt: Write error');
    });
  });
});